...
export function listRepositories(params: {
  ...
  if (params.selectedOnly !== undefined) query.set("selectedOnly", String(params.selectedOnly));
  if (params.refresh) query.set("refresh", "true");
  return apiFetch<Paginated<GitHubRepositoryDto>>(`/api/github/repositories?${query}`);
}

export function updateRepositoryAccess(
  input: UpdateRepositoryAccessRequest,
): Promise<UpdateRepositoryAccessResponse> {
  return apiFetch<UpdateRepositoryAccessResponse>("/api/github/repositories/access", {
    method: "PATCH",
...
export function authorizeRepository(
  ...
  return apiFetch<SessionRepositoryAuthorization>("/api/github/repositories/authorize", {
    method: "POST",
...
export function selectionLabel(selection: RepositorySelection): string {
  return selection === "all" ? "All repositories" : "Only selected repositories";
}
...
export const ERROR_MESSAGES: Record<string, string> = {
  ...
  GITHUB_INSTALLATION_REMOVED: "The GitHub App installation was removed or suspended.",
  GITHUB_REPOSITORY_UNAVAILABLE: "That repository is not available through your installation.",
  GITHUB_REPOSITORY_NOT_SELECTED: "That repository is not enabled for this application.",
  GITHUB_INSUFFICIENT_PERMISSIONS: "The GitHub App lacks the required permissions.",
  ...
  GITHUB_UNAUTHORIZED_REPOSITORY: "One or more repositories are not yours to select.",
};
